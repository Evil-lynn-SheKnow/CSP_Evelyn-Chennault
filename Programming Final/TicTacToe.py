...
def make_move(player):    
    ...
    for turn in range(9):#Maximum 9 moves    
        ...
        if turn % 2 == 0:
            ...
            make_move(player)#Make the player's move    (josy)
 
display_board(board)#Show final board (zoey)
      ...
      if player == "X": #maisha did these conditionals
          while True:
              ...
              if 0 <= move <= 8 and board[move] == " ":
                  ...
                  print("invalid bro try once more.") #Maisha (conditionals)
      else:
          move = computer(board) #the computer is putting move down (Zoey)
          board[move] = player
          print(f"Computer's move: {move + 1}")
      if check_win(board, player): #checks to verify the win (maisha) and displays the board to say that the player won if the player won
          display_board(board)
